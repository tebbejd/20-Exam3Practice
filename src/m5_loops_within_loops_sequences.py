...
def run_test_integers():
    ...
    # Test 2:
    expected = [3, 1, 4, 10, 10, 10, 0, 11, 9, 2]
    answer = integers([(3, 1, 4),
                       (10, 10, 10),
                       [0, 11, '.25', 9],
                       'hello',
...
def integers(sequence_of_sequences):
    ...
    for k in range(len(sequence_of_sequences)):
        for l in range(len(sequence_of_sequences[k])):
            if type(sequence_of_sequences[k][l]) is int:
                list += [sequence_of_sequences[k][l]]
    return list
...
def run_test_big_letters():
    ...
    # Test 1:
    expected = 'UHHI'
    answer = big_letters(['gUiHu', [5, 4, 5, 6, 6, 77, 78, 8], 'goooooood', 'HI'])
    print('Expected is:', expected)
    print('Actual is:  ', answer)
        ...
        for k in range(len(sequence_of_sequences[j])):
            if type(sequence_of_sequences[j]) is str:
                if sequence_of_sequences[j][k].isupper() == True:
                    string += sequence_of_sequences[j][k]
    return string
